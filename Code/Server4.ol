...
//5 output port a tutti i server.Anche server 4
...
define sendRequestVote
{
	...
	*/
	...
	//if(risposta4 == 1){ global.serverNumeroVoti ++};
	if(risposta5 == 1){ 
		global.serverNumeroVoti ++;
...
init{
	...
	global.iD_Server = 4; //ID del server, ogni server ha un ID diverso
	...
	startServer@ToServer4(t)  //Richiamo l'operazione startServer che mi fa partire il processo del server (loop infinito).
}
