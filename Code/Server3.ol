...
//5 output port a tutti i server.Anche server 3
...
define sendRequestVote
{
	...
	*/
	...
	//if(risposta3 == 1){ global.serverNumeroVoti ++};
	if(risposta4 == 1){ 
		global.serverNumeroVoti ++;
...
*/
define infiniteLoop{
	//Faccio un for che tende all'infinito
	for ( i = 0, i>-1, i++ ){
		...
		*/
		if( global.Timer >= global.TimeForNextTimeout && global.stato != 3){
			...
			println@Console( "SONO CANDIDATE" )();
		  	...
		  	println@Console( "RV >>>>" )();
		  	settaNuovoTimeout;	//Setta un nuovo Election Timeout
...
init{
	...
	global.iD_Server = 3; //ID del server, ogni server ha un ID diverso
	...
	startServer@ToServer3(t)  //Richiamo l'operazione startServer che mi fa partire il processo del server (loop infinito).
}
