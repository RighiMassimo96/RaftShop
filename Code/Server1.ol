...
/*
Costruisco 5 output port per comunicare con tutti i server.
Anche server 1 ha una output port poichè comunica con se stesso.
*/
...
{
	//Il server che si vuole candidare invia agli altri server due info: il suo ID e il valore del suo termine
    ...
    */
	scope( s )
	{
	  ...
	  sendRequestVote@ToServer2 (termineIdServer)(risposta2)|
	...
	*/
	if(risposta2 == 1){
		...
		println@Console( "Server 2 ha votato per me" )()
	};
	if(risposta3 == 1){
		global.serverNumeroVoti ++;
...
define sendHeartBeat{
	...
	scope( s )
	{
	  ...
	  sendHeartBeat@ToServer2 (termineIdServer)(risposta2)|
...
/*
Questa procedura serve per aggiornare lo stato e le variabili globali di ogni server ogni volta che 
...
define sincronizzaServers{
		...
		*/
		...
		scope( s )
		{
			...
			sincronizzaServers@ToServer2 (global.syncro)(risposta2)|
...
init{
	...
	global.iD_Server = 1; //ID del server, ogni server ha un ID diverso
